...
let check_variable_name name l = if List.mem name exclusion_names then 
begin printf "Exception: Line %d, cannot name variable %s: forbidden name (reserved keyword)\n" l name;
exit 1; 
end 
else if Read.is_numeric (String.make 1 (String.get name 0)) 
then begin printf "Exception: Line %d, cannot name variable %s: invalid name\n" l name;
exit 1; end;;
